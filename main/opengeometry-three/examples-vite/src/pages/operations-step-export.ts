...
function formatReport(report: StepExportReport, bytes: number, filename: string): string {
  return [
    `File: ${filename}`,
    `Bytes: ${bytes}`,
...
bootstrapExample({
  title: "Operation: STEP Export",
  description:
    "Select a shape, tune parameters, and export a STEP Part-21 model with best-effort reporting.",
  build: ({ scene }) => {
    const manager = new OGSceneManager();
    const sceneId = manager.createScene("step-export-scene");
    const entityId = "step-shape";
    ...
    heading.textContent = "STEP Export";
    ...
    summary.textContent = "Pick a shape and export the currently displayed model as STEP.";
      ...
      try {
        ...
        switch (state.shape) {
          case "cuboid": {
            ...
            fileName = "opengeometry-cuboid.step";
          ...
          case "cylinder": {
            ...
            fileName = "opengeometry-cylinder.step";
          ...
          case "sphere": {
            ...
            fileName = "opengeometry-sphere.step";
          ...
          case "wedge": {
            ...
            fileName = "opengeometry-wedge.step";
        ...
        manager.addBrepEntityToScene(sceneId, entityId, kind, JSON.stringify(brepData));
    ...
    exportButton.textContent = "Export STEP";
    ...
    exportButton.dataset.filename = "opengeometry-cuboid.step";
    ...
    reportNode.textContent = "Generate geometry to enable STEP export.";
    ...
    exportButton.addEventListener("click", () => {
      try {
        ...
        const filename = exportButton.dataset.filename ?? "opengeometry-shape.step";

        downloadText(result.text, filename, "application/step");
        reportNode.textContent = formatReport(report, new TextEncoder().encode(result.text).length, filename);
      } catch (error) {

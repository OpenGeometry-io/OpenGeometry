    ...
    pub fn generate_brep(&mut self) {
        if self.points.len() < 3 {
            web_sys::console::log_1(
                &"Polygon must have at least 3 points to generate geometry.".into(),
            );
            return;
        }
        ...
        self.brep
            .faces
            .push(Face::new(self.brep.get_face_count() as u32, Vec::new()));
        ...
        for (i, point) in self.points.iter().enumerate() {
            ...
            let edge = { vec![i as u32, ((i + 1) % self.points.len()) as u32] };

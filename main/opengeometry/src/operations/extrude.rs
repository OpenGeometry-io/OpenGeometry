...
use crate::{
    brep::{edge, Brep, Edge, Face, Vertex},
    geometry::basegeometry::BaseGeometry,
    utility::geometry::{Geometry, Geometry_Holes},
};
...
pub fn extrude_polygon_by_buffer_geometry(geom_buf: BaseGeometry, height: f64) -> Geometry {
    ...
    for i in 0..current_length {
        let edge = { vec![i as u8, ((i + 1) % ccw_vertices.len()) as u8] };
        buf_edges.push(edge);
    }
    ...
    for index in 0..ccw_vertices.len() {
        ...
        let edge = { vec![index as u8, buf_vertices.len() as u8 - 1] };
    ...
    for i in current_length..buf_vertices.len() {
        if i < buf_vertices.len() - 1 {
            let edge = { vec![i as u8, (i + 1) as u8] };
            buf_edges.push(edge);
        } else {
            let edge = { vec![i as u8, (current_length) as u8] };
            buf_edges.push(edge);
        }
    }
...
pub fn extrude_brep_face(brep_face: Brep, height: f64) -> Brep {
    ...
    extruded_brep
        .faces
        .push(Face::new(extruded_brep.get_face_count() as u32, Vec::new()));
    for i in 0..ccw_vertices.len() {
        let vertex = ccw_vertices[i].clone();
        extruded_brep
            .vertices
            .push(Vertex::new(extruded_brep.get_vertex_count() as u32, vertex));
    ...
    for i in 0..current_length {
        ...
        for j in 0..face.len() {
            let edge = { vec![face[j], face[(j + 1) % face.len()]] };
            extruded_brep.edges.push(Edge::new(
                extruded_brep.get_edge_count() as u32,
                edge[0],
                edge[1],

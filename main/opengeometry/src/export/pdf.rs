...
use crate::export::projection::{
    project_brep_to_scene, CameraParameters, HlrOptions, Path2D, Scene2D, Segment2D, Vec2,
};
use printpdf::*;
use std::fs::File;
...
const DEFAULT_LINE_WIDTH_MM: f64 = 0.25;
...
/// Project a BRep with camera parameters and export to vector PDF
...
fn draw_path_to_layer(
    ...
    if let Some(width) = path.stroke_width {
        layer.set_outline_thickness(width * METERS_TO_MM * scale);
    }
...
/// Transform a point from scene coordinates to PDF coordinates

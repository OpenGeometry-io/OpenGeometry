...
This guide walks through a minimal OpenGeometry + Three.js setup for browser-based CAD, AEC/BIM, and other geometry-heavy web tools.
...
Initialize the OpenGeometry WASM module once before creating shapes:
...
<Note>
  Call `OpenGeometry.create(...)` before constructing `Vector3` or any wrapper.
</Note>

<Note>
  With Vite, copy `node_modules/opengeometry/opengeometry_bg.wasm` to `public/opengeometry_bg.wasm`
  so it is served at `/opengeometry_bg.wasm`.
</Note>
...
<Info>
  The `Cuboid` class extends `THREE.Mesh`, so it can be added directly to Three.js scenes. Other
  3D shape wrappers work the same way, while primitive wrappers such as `Line` and `Arc` are
  line-based instead of mesh-based.
</Info>
...
OpenGeometry provides many built-in shapes for browser CAD and AEC/BIM workflows. The snippets
below assume you have already initialized `OpenGeometry`.
...
<Note>
  For more complete reference examples, use the standalone demo catalog at
  [demo.opengeometry.io](https://demo.opengeometry.io/) or browse the example guide at

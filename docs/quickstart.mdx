...
<Info>
  `Cuboid` is a `THREE.Group` containing a transient surface mesh and analytic feature outline, so
  it can be added directly to Three.js scenes. Primitive wrappers such as `Line` and `Arc` are
  line-based.
</Info>

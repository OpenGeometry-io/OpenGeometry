...
renderable `BooleanResult` mesh with access to the output B-Rep, a structured report, and freeform
conversion helpers. These boolean paths are central to browser CAD and AEC/BIM workflows such as
wall/opening subtraction and solid-to-solid modeling.
...
`booleanSubtraction(lhs, rhs, options?)` stays a binary helper. Use the wrapper-level
`shape.subtract([operands], options?)` API when you want to cut a host with multiple cutters in one
call.

### shape.subtract([...])

Shape wrappers expose an array-only subtract convenience method:

```ts
const result = wall.subtract([doorOpening, windowOpening], options);
```

Pass `[operand]` even when you only have one cutter. `Opening.subtractFrom(host, options?)`
remains binary so the opening stays the cutting tool.
...
This means you can pass OpenGeometry shape wrappers (like `Cuboid`, `Opening`, and `Solid`) directly.
That makes `polygon.extrude(height)` a first-class boolean input for browser CAD and AEC workflows.
...
<ResponseField name="toFreeform" type="(options?: { id?: string; placement?: { translation?: Vector3; rotation?: Vector3; scale?: Vector3 } }) => FreeformGeometry">
  Converts the boolean output into a freeform geometry object for direct editing.
</ResponseField>
...
### Cut a wall with two openings

Use the array-only wrapper subtraction when the host owns multiple cutters:
...
const windowOpening = new Opening({
  center: new Vector3(1.35, 1.75, 0),
  width: 1.3,
  height: 1.1,
  depth: 0.4,
  color: 0xffffff,
...
const cutWall = wall.subtract([doorOpening, windowOpening], {
  color: 0x10b981,
  outline: true,
  fatOutlines: true,
  outlineWidth: 2,

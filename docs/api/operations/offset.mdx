...
<ParamField path="acute_threshold_degrees" type="f64" default="35.0">
  The interior angle threshold (in degrees) for sharp-corner fallback. Outer corners bevel when `bevel` is enabled and the angle is at or below the threshold. Open-path inner corners clip instead of mitering when the angle is at or below the threshold. Valid range: 1.0 to 179.0 degrees. Higher values apply beveling and clipping more aggressively.
</ParamField>
...
<ResponseField name="beveled_vertex_indices" type="Vec<u32>">
  Indices of vertices in the original path where outer-corner beveling was applied. Open-path inner-corner clipping can also add points, but those vertices are not included here because they are not bevels.
</ResponseField>
...
3. **Corner Processing**:
   ...
   - **Inner corners** (open paths): Mitered for broader turns, clipped below the threshold to prevent spikes
...
- **Open paths, angle > threshold**: Mitered to intersection point
- **Open paths, angle ≤ threshold**: Clipped with two vertices to prevent long spikes

## Implementation Details

### Source Location

`main/opengeometry/src/operations/offset.rs`
...
- [Offset demo](https://demo.opengeometry.io/operations/offset.html)
- [Wall from offsets demo](https://demo.opengeometry.io/operations/wall-from-offsets.html)

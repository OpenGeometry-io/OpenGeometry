...
Returns a `Brep` (Boundary Representation) using OpenGeometry's current half-edge topology schema
(see [BRep](/concepts/brep)). At a high level, the result contains:

- Vertices for each profile section placed along the path
- Side faces connecting successive sections
- Optional end caps (for open paths) when enabled
...
Try it in the browser: [Sweep path/profile demo](https://demo.opengeometry.io/operations/sweep-path-profile.html).

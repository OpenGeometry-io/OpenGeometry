...
<ResponseField name="fatOutlines" type="boolean">
  When enabled, outlines are rendered using a wide-line implementation (instead of thin lines).
</ResponseField>
...
<ResponseField name="outlineMesh" type="THREE.LineSegments | object | null">
...
Subtracts another boolean operand from this opening volume and returns a renderable result mesh.

```typescript
subtract(operand, options?): BooleanResult
```

### subtractFrom()

Uses this opening volume to cut a host shape and returns a renderable result mesh.

```typescript
subtractFrom(host, options?): BooleanResult
```

<Tip>
For architectural workflows (doors/windows cutting walls), `subtractFrom(...)` is usually the clearest API.
</Tip>
...
```typescript
import { Opening, Vector3 } from "opengeometry";
...
- [Boolean operations](/api/operations/boolean-operations) - Union, intersection, and subtraction
...
Try it in the browser: [Opening demo](https://demo.opengeometry.io/shapes/opening.html).

...
Subtracts another boolean operand (such as an `Opening`) from this cuboid and returns a renderable result mesh.
...
const cut = wall.subtract(opening, { outline: true, fatOutlines: true, outlineWidth: 2 });
scene.add(cut);
```
...
Try it in the browser: [Cuboid demo](https://demo.opengeometry.io/shapes/cuboid.html).
